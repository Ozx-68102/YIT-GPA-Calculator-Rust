...
8. **User-Agent 与「服务器模式」**（用户后续补充）  
   当前 `scraping.rs` 中 `USER_AGENT` 为进程级单一值（`lazy_static!` + `Mutex<String>`），每次启动随机一个
   UA、进程内所有请求共用。设计前提是「本地脚本、用一次就关」；若有人把程序当服务器公开给多人访问，所有人会共用同一 UA，教务反爬风险高。
   **若未来开发出「服务器模式」或多人共用同一进程的场景，这段逻辑必须改**（例如改为每会话或每请求独立
   UA），具体实现可留待日后，但需在问题清单中保留此约束。
...
| 6 | 服务器模式与 UA：若未来支持多用户/服务器部署，必须改为「每会话或每请求独立 UA」，当前 `lazy_static` 单 UA 逻辑不可沿用 | 问题 8、用户补充       |
