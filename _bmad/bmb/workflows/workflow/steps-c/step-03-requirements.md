...
To gather comprehensive requirements through conversation, building on the classification decisions, and document them
in a standardized format for the design phase.
...
- "We'll use a minimal template with basic frontmatter. The workflow will build the document section by section, with a
  final polish step to optimize flow."
...
**Master Rule:** Requirements build on classification. Use the standardized template so the next steps can read
consistent data.
