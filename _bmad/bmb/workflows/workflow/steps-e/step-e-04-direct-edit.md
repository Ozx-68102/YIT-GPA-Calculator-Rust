...
Apply direct user-requested changes to the workflow, loading relevant standards and checking for non-compliance during
editing.
...
ONLY WHEN all direct changes are applied (or documented) and user confirms, will you then load and read fully
`{nextStepFile}` to execute.
...
**Master Rule:** Load standards for each change type. Check compliance BEFORE applying changes. Offer to fix
non-compliance when found.
