...
To validate that EVERY step file's frontmatter follows the frontmatter standards - correct variables, proper relative
paths, NO unused variables.
...
Subprocess returns file name, frontmatter compliance status, unused variables found, path violations, and overall
status (PASS/FAIL). Include specific variable names and violation details for documentation.
...
**Master Rule:** Validation is systematic and thorough using subprocess optimization. DO NOT BE LAZY. For EACH file,
launch a subprocess that validates frontmatter, checks variable usage, validates paths, and returns findings.
