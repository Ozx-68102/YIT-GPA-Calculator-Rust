...
To check if the workflow has proper validation steps when validation is critical - validation steps should load from
validation data and perform systematic checks.
...
**Context savings:** Each subprocess returns analysis (~30 lines), not full step file (~200 lines). Parent gets
structured findings, not file contents.
...
**Whether validation is required:** Indicate if this workflow needs validation steps based on its domain type (
critical/compliance/safety workflows vs. creative/exploratory ones)
...
**"DO NOT BE LAZY" language presence:** For each validation step, note whether anti-lazy language is present and what it
says
...
Update {validationReportFile} - replace "## Validation Design Check *Pending...*" with actual findings from subprocess
aggregation.
...
**Master Rule:** Validation is systematic and thorough. DO NOT BE LAZY. Check validation steps thoroughly. Auto-proceed
