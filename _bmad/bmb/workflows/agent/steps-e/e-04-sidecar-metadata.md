...
Review the agent's hasSidecar decision and metadata, and plan any changes. If edits involve sidecar conversion, identify
the implications.
...
- Explain implications:
    ...
    - true → false: Remove sidecar fields; if critical_actions only has sidecar references, remove section; otherwise
      keep non-sidecar critical_actions
...
ONLY WHEN [C continue option] is selected and [metadata changes documented], will you then load and read fully
`{nextStepFile}` to execute and begin persona planning.
