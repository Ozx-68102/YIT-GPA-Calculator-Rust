...
through collaborative metric definition aligned with product vision and user value.
...
- Available context: Current document and frontmatter from previous steps, product vision and target users already
  defined
...
"I've defined success metrics that will help us track whether {{project_name}} is creating real value for users and
...
- IF A: Read fully and follow: {advancedElicitationTask} with current metrics content to dive deeper into success metric
  insights
...
and [success metrics finalized and saved to document with frontmatter updated], will you then read fully and follow:
`{nextStepFile}` to begin MVP scope definition.
